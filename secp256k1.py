####
# Code source: https://github.com/mohanson/cryptography-python
####
import time
import random
...
if __name__ == '__main__':
    ...
    print('G.y:{}:'.format(G.y))
    print('G.x.x:{:02x}:'.format(G.x.x))
    print('G.y.x:{}:'.format(G.y.x))
    print('G.x.x:{:02x}:'.format(G.x.x.to_bytes(32, byteorder="big")))
    p = G * Fr(42)
    q = G * Fr(24)
    r = Pt(p.x, -p.y)
    assert p + q == G * Fr(66)
